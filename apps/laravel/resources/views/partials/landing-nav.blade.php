<style>
    ...
    .mobile-whatsapp-link { display: flex; align-items: center; justify-content: center; gap: 10px; min-height: 48px; border-radius: 999px; color: #fff; background: rgba(37,211,102,0.12); border: 1px solid rgba(37,211,102,0.35); text-decoration: none; font-size: 14px; font-weight: 600; }
    ...
    @media (max-width: 767px) {
        .header-lang-switcher, .header-whatsapp-btn { display: none !important; }
    }
</style>
...
<!-- Navigation -->
<nav class="floating-site-nav" aria-label="{{ __('landing.nav.aria_main') }}">
    <div class="relative max-w-7xl mx-auto container-pad floating-site-nav-frame">
        <div class="flex justify-between items-center h-16 sm:h-[68px]">
            <a href="{{ route('landing') }}" class="flex items-center gap-3" aria-label="Kosta - {{ __('landing.nav.home') }}">
            ...
            <div class="hidden md:flex items-center gap-8">
                <a href="/#servicios" class="nav-link text-sm" @if(request()->is('servicios*')) style="color:#fff;" @endif>{{ __('landing.nav.services') }}</a>
                <a href="{{ route('portfolio') }}" class="nav-link text-sm" @if(request()->routeIs('portfolio')) style="color:#fff;" @endif>{{ __('landing.nav.portfolio') }}</a>
                <a href="{{ route('contact') }}" class="nav-link text-sm" @if(request()->routeIs('contact')) style="color:#fff;" @endif>{{ __('landing.nav.contact') }}</a>
            </div>

            <div class="flex items-center gap-2.5 sm:gap-3">
                <!-- Language Toggle (Desktop) -->
                <div class="lang-switcher header-lang-switcher" role="group" aria-label="{{ __('landing.nav.lang_switch') }}">
                    <a href="{{ route('locale.switch', 'es') }}" class="lang-btn {{ app()->getLocale() === 'es' ? 'active' : '' }}" aria-label="{{ __('landing.nav.lang_es') }}" title="{{ __('landing.nav.lang_es') }}">ES</a>
                    <span class="lang-divider">/</span>
                    <a href="{{ route('locale.switch', 'en') }}" class="lang-btn {{ app()->getLocale() === 'en' ? 'active' : '' }}" aria-label="{{ __('landing.nav.lang_en') }}" title="{{ __('landing.nav.lang_en') }}">EN</a>
                </div>
                ...
                <a href="https://wa.me/573113894136" target="_blank" rel="noopener noreferrer" class="nav-whatsapp-btn header-whatsapp-btn flex" aria-label="{{ __('landing.nav.whatsapp_aria') }}">
                ...
                <button class="hamburger-btn md:hidden" id="menuToggle" aria-label="{{ __('landing.nav.menu_open') }}" aria-expanded="false" aria-controls="mobileMenu">
...
<!-- Mobile Menu -->
<div id="mobileMenu" class="mobile-menu" role="dialog" aria-modal="true" aria-label="{{ __('landing.nav.menu_aria') }}" hidden>
    <div class="mobile-drawer">
        <div class="mobile-drawer-header">
            ...
            <button class="w-10 h-10 flex items-center justify-center text-white/60 hover:text-white transition-colors rounded-full hover:bg-white/10" id="menuClose" aria-label="{{ __('landing.nav.menu_close') }}">
        ...
        <div class="mobile-drawer-body">
            <nav class="flex flex-col">
                <a href="{{ route('landing') }}" class="mobile-nav-link" @if(request()->routeIs('landing')) style="color:var(--accent-red)" @endif>{{ __('landing.nav.home') }}</a>
                <a href="/#servicios" class="mobile-nav-link" @if(request()->is('servicios*')) style="color:var(--accent-red)" @endif>{{ __('landing.nav.services') }}</a>
                <a href="{{ route('portfolio') }}" class="mobile-nav-link" @if(request()->routeIs('portfolio')) style="color:var(--accent-red)" @endif>{{ __('landing.nav.portfolio') }}</a>
                <a href="{{ route('contact') }}" class="mobile-nav-link" @if(request()->routeIs('contact')) style="color:var(--accent-red)" @endif>{{ __('landing.nav.contact') }}</a>
            </nav>

            <div class="flex items-center justify-between py-5 border-t border-white/10 mt-6">
                <span class="text-sm font-medium text-white/60">{{ __('landing.nav.lang_switch') }}</span>
            ...
            <div class="pt-6 flex flex-col gap-3 mt-auto">
                <a href="https://wa.me/573113894136" target="_blank" rel="noopener noreferrer" class="mobile-whatsapp-link" aria-label="{{ __('landing.nav.whatsapp_aria') }}">
                    <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                    </svg>
                    WhatsApp
                </a>
                <a href="{{ route('contact') }}" class="btn-primary justify-center">{{ __('landing.nav.start_project') }}</a>
            </div>
        </div>
    </div>
</div>
